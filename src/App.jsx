...
const DevProvider = ({ children }) => {
    const [managedVideos, setManagedVideos] = useLocalStorage('bdukasi-dev-videos-v2', [{id: "Ld2M9q9sC-s", title: "Apa itu Paradoks?", embedUrl: "https://www.youtube.com/embed/Ld2M9q9sC-s"}]);
...
const AppProvider = ({ children }) => {
    ...
    const fetchBankSoal = useCallback(async (topic, count) => {
        ...
        setIsLoading(true); setLoadingMessage(`Kak Spenta AI sedang membuat ${count} soal...`);
        ...
        } catch(err) { setPage('dashboard'); } finally { setIsLoading(false); }
    }, [level, track, subject]);
    
    const value = { page, setPage, screen, setScreen, level, setLevel, track, setTrack, subject, setSubject, learningData, isLoading, loadingMessage, isSidebarOpen, setSidebarOpen, pwaInstallPrompt, setPwaInstallPrompt, fetchLearningMaterial, fetchBankSoal, bankSoal, recommendations, setRecommendations };
...
const MainApp = () => {
    ...
    const { setPwaInstallPrompt } = useContext(AppContext);
                    ...
                    <div className="bg-white dark:bg-slate-700 p-4 rounded-2xl shadow-2xl max-w-sm flex items-start gap-4 border border-slate-200 dark:border-slate-600">
                        ...
                        <div>
                            ...
                            <div className="mt-4 flex gap-2">
                                <button onClick={handleInstall} className="px-4 py-2 text-sm bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700">Instal</button>
                                <button onClick={() => setPwaInstallPrompt(null)} className="px-4 py-2 text-sm bg-transparent text-slate-500 font-medium rounded-lg hover:bg-slate-100 dark:hover:bg-slate-600">Nanti</button>
                            </div>
                        </div>
                    </div>
                </div>
...
const LoadingScreen = ({ message }) => (
    <div className="fixed inset-0 bg-sky-50 dark:bg-slate-900 z-50 flex flex-col items-center justify-center gap-6">
        <div className="animate-bounce"><Logo className="w-24 h-24" /></div>
...
const LandingPage = () => {
    ...
    const features = [
        { icon: BrainCircuit, title: "Guru AI Pribadi", description: "Dapatkan penjelasan materi dan jawaban dari AI canggih kapan saja." },
        { icon: VideoIcon, title: "Video Terkurasi", description: "Belajar dari video pilihan yang relevan dengan kurikulum dan mudah dipahami." },
        { icon: BookMarked, title: "Bank Soal Adaptif", description: "Latih pemahamanmu dengan ribuan soal yang tingkat kesulitannya menyesuaikan." },
    ];
    ...
    return (
        <div className="bg-sky-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 min-h-screen">
            ...
            <header className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-10">
                <div className="flex items-center gap-2"><Logo className="w-10 h-10"/><span className="text-2xl font-bold text-slate-800 dark:text-white">Bdukasi</span></div>
                <button onClick={loginWithGoogle} disabled={loading} className="px-5 py-2 bg-blue-500 text-white font-semibold text-sm rounded-full shadow-sm hover:bg-blue-600 transition-colors">Masuk</button>
            </header>

            {/* Hero Section */}
            <section className="min-h-screen flex flex-col items-center justify-center p-4 overflow-hidden relative text-center">
                <div className="absolute -top-40 -left-40 w-96 h-96 bg-blue-200/50 rounded-full mix-blend-multiply filter blur-xl animate-blob dark:opacity-30"></div>
                <div className="absolute -bottom-40 -right-40 w-96 h-96 bg-teal-200/50 rounded-full mix-blend-multiply filter blur-xl animate-blob animation-delay-2000 dark:opacity-30"></div>
                
                <div className="z-10 bg-white/30 dark:bg-slate-800/30 backdrop-blur-lg p-8 rounded-3xl shadow-2xl shadow-blue-500/10 max-w-3xl">
                    <h1 className="text-4xl sm:text-6xl font-extrabold text-slate-900 dark:text-white tracking-tight">Teman Belajar <span className="text-blue-500">Terbaikmu</span>.</h1>
                    <p className="mt-6 text-lg text-slate-600 dark:text-slate-300 max-w-xl mx-auto">Ubah cara belajarmu jadi lebih seru dan personal dengan AI, dirancang khusus untuk pelajar Indonesia.</p>
                    <button onClick={loginWithGoogle} disabled={loading} className="mt-8 px-8 py-4 bg-blue-500 text-white font-bold text-lg rounded-full shadow-lg hover:bg-blue-600 transform hover:scale-105 transition-all flex items-center gap-3 disabled:bg-slate-400">
            ...
            <section className="py-20 px-4 bg-white dark:bg-slate-800">
                <div className="max-w-5xl mx-auto text-center">
                    <h2 className="text-3xl font-bold mb-4">Kenapa Memilih Bdukasi?</h2>
                    <p className="text-slate-600 dark:text-slate-400 mb-12 max-w-2xl mx-auto">Kami menyediakan alat belajar modern yang membuat setiap materi menjadi lebih mudah dan menyenangkan.</p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {features.map((feature, i) => (
                            <div key={i} className="p-8 bg-sky-50 dark:bg-slate-700/50 rounded-2xl">
            ...
            {/* Mission Section */}
             <section className="py-20 px-4">
                <div className="max-w-3xl mx-auto text-center">
                    <div className="p-4 inline-block bg-teal-100/80 dark:bg-teal-900/50 rounded-full mb-4">
                       <Target className="w-10 h-10 text-teal-500" />
                    </div>
                    <h2 className="text-3xl font-bold mb-4">Misi Kami</h2>
                    <p className="text-slate-600 dark:text-slate-400 text-lg leading-relaxed">
                        Misi kami adalah untuk membuat pendidikan berkualitas tinggi dapat diakses oleh semua pelajar di Indonesia, di mana pun mereka berada. Kami percaya bahwa dengan teknologi yang tepat, belajar bisa menjadi sebuah petualangan yang menyenangkan dan memberdayakan setiap individu untuk meraih potensi terbaiknya.
                    </p>
                </div>
            </section>
            
            {/* Footer */}
            <footer className="w-full text-center p-8 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-sm">
...
// ... Sisa komponen (DashboardPage, DeveloperDashboard, dll.) tetap sama ...
const DashboardPage = () => {
    const { user } = useContext(AuthContext);
    ...
    return (
        <AnimatedScreen customKey="dashboard">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <DashboardCard icon={<BookMarked size={32} className="text-blue-500"/>} title="Bank Soal" description="Latihan soal tak terbatas." onClick={() => setPage('belajar')} />
                <div className="md:col-span-2"><DailyMissionCard /></div>
                <LeaderboardCard />
            </div>
        </AnimatedScreen>
    );
    ...
    return (
        <AnimatedScreen key="developer">
            <div className="flex justify-between items-start mb-6">
                <div><h1 className="text-3xl font-bold">Developer Dashboard</h1><p className="text-slate-500">Alat dan statistik khusus untuk tim Bgune.</p></div>
                <div className="flex items-center gap-2 text-xs font-mono p-2 bg-slate-200 dark:bg-slate-700 rounded"><span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>ADMIN_MODE</div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8"><div className="lg:col-span-2 space-y-8"><VideoManagement /></div><div className="lg:col-span-1 space-y-8"><DevStats /><DevConsole /></div></div>
        </AnimatedScreen>
    );
};
const SettingsPage = () => {
        ...
        <AnimatedScreen key="settings">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <InfoCard title="Tampilan">
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700"><label className="font-semibold block mb-2">Ukuran Font</label><div className="flex gap-2">{['sm', 'base', 'lg'].map(size => <button key={size} onClick={() => setFontSize(size)} className={`px-4 py-2 rounded-lg font-semibold text-sm ${fontSize === size ? 'bg-blue-500 text-white' : 'bg-slate-200 dark:bg-slate-700'}`}>{size.charAt(0).toUpperCase() + size.slice(1)}</button>)}</div></div>
                </InfoCard>
                ...
                <InfoCard title="Akun"><button onClick={logout} className="w-full text-left p-3 bg-red-500 text-white font-bold rounded-lg flex items-center gap-2 hover:bg-red-600"><LogOut size={20}/> Keluar</button></InfoCard>
            </div>
        </AnimatedScreen>
    )
};
...
const LearningFlow = () => {
    ...
    const screens = {
        ...
        trackSelection: <TrackSelectionScreen />,
        subjectSelection: <SubjectSelectionScreen />,
        subjectDashboard: <SubjectDashboardScreen />,
        lesson: <LearningMaterialScreen />,
        bankSoal: <BankSoalScreen />,
    };
...
const TrackSelectionScreen = () => { 
    ...
    return (
        <AnimatedScreen customKey="track">
            ...
            <div className="text-center"><h1 className="text-4xl font-bold mb-12">Pilih Jurusan</h1><div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-xl mx-auto">{Object.keys(curriculum.SMA.tracks).map((trackName) => <button key={trackName} onClick={() => { setTrack(trackName); setScreen('subjectSelection'); }} className="p-8 bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700/50 rounded-2xl shadow-sm hover:shadow-blue-500/20 hover:border-blue-500 hover:-translate-y-2 transition-all text-2xl font-bold">{trackName}</button>)}</div></div>
        </AnimatedScreen>
    );
};
...
const SubjectDashboardScreen = () => {
    ...
    return (
        <AnimatedScreen customKey="subject-dashboard">
            ...
            <div className="w-full max-w-2xl mx-auto my-12"><form onSubmit={handleSearchSubmit} className="relative"><input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Ketik topik untuk dipelajari..." className="w-full pl-6 pr-16 py-4 text-lg bg-white dark:bg-slate-700 border-2 border-slate-300 dark:border-slate-600 rounded-full focus:ring-4 focus:ring-blue-500/50 focus:border-blue-500 outline-none"/><button type="submit" className="absolute right-2.5 top-1/2 -translate-y-1/2 p-2.5 bg-blue-600 text-white rounded-full hover:bg-blue-700"><Search className="w-6 h-6" /></button></form></div>
            <BankSoalGenerator fetchBankSoal={fetchBankSoal} />
        </AnimatedScreen>
    );
};
const BankSoalGenerator = ({ fetchBankSoal }) => {
    const [topic, setTopic] = useState('');
    const [count, setCount] = useState(5);
    const handleSubmit = (e) => { e.preventDefault(); if (topic.trim()) fetchBankSoal(topic, count); };
...
const VideoManagement = () => {
    ...
    const handleDelete = (videoId) => { setManagedVideos(managedVideos.filter(v => v.id !== videoId)); addLog('warn', `Video ID ${videoId} deleted.`); };
    return (
...
const DevConsole = () => {
    ...
    const handleClearVideos = () => { if(window.confirm('Yakin ingin hapus semua video?')) { setManagedVideos([]); addLog('error', 'All managed videos have been reset.'); } }
    return (
        <InfoCard title="Konsol Developer">
             <div className="bg-black text-white font-mono text-xs rounded-lg p-4 h-64 overflow-y-scroll flex flex-col-reverse"><div className="space-y-1">{devLogs.map((log, i) => (<p key={i}><span className="text-slate-500">{log.time}&gt; </span><span className={log.type === 'error' ? 'text-red-400' : log.type === 'warn' ? 'text-amber-400' : 'text-green-400'}>{log.msg}</span></p>))}</div></div>
            <div className="mt-4 space-y-2"><button onClick={handleClearVideos} className="w-full p-2 text-sm text-white bg-red-600 rounded-md hover:bg-red-700">Reset Semua Video</button></div>
        </InfoCard>
    )
};
const Sidebar = () => {
...
const DailyMissionCard = () => (<InfoCard title="Misi Harian"><div className="space-y-3"><p className="flex items-center gap-3"><CheckCircle className="text-green-500"/> Tonton 1 video belajar</p><p className="flex items-center gap-3"><CheckCircle className="text-green-500"/> Kerjakan 5 soal latihan</p><p className="flex items-center gap-3 text-slate-400"><X className="w-5 h-5"/> Tanya 1 pertanyaan ke AI</p></div></InfoCard>);
...
styleSheet.innerText = `
    ...
    .font-size-sm { font-size: 0.9rem; } .font-size-base { font-size: 1rem; } .font-size-lg { font-size: 1.1rem; }
    ...
    .prose {max-width: 100%;} .prose h1, .prose h2, .prose h3, .prose p, .prose strong, .prose ul, .prose ol, .prose li {color: inherit;}
